...
function platformApiKey() {
  ...
  if (Platform.OS === "ios") {
    return env.EXPO_PUBLIC_REVENUECAT_IOS_API_KEY ?? env.EXPO_PUBLIC_REVENUECAT_IOS_KEY;
  }
  if (Platform.OS === "android") {
    return env.EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY ?? env.EXPO_PUBLIC_REVENUECAT_ANDROID_KEY;
  }
  return undefined;
}
...
export function RevenueCatProvider({ children }: { children: React.ReactNode }) {
  ...
  const available = (Platform.OS === "ios" || Platform.OS === "android") && !isPreviewEnvironment();
  ...
  const refresh = async () => {
    if (!available) {
      setError(new IntegrationError("revenuecat_preview", "Real purchases require a development build."));
      return;
    }
    if (!user) return;
    ...
    try {
      ...
      if (!apiKey) {
        throw new IntegrationError("revenuecat_not_configured", "Missing RevenueCat public SDK key.");
      }
      ...
      if (!offerings.current) {
        throw new IntegrationError("offering_unavailable", "RevenueCat has no current offering.");
      }
  ...
  const purchase = async (tier: PaidTier): Promise<PurchaseResult> => {
    ...
    try {
      ...
      const nextError = toIntegrationError(cause, "purchase_failed", "The purchase could not be completed.");
  ...
  const restore = async () => {
    if (!available) {
      throw new IntegrationError("revenuecat_preview", "Restore requires a development build.");
    }
    try {
      await Purchases.restorePurchases();
      await syncProfileTier();
    } catch (cause) {
      throw toIntegrationError(cause, "restore_failed", "Purchases could not be restored.");
    }
  };

  const manage = async () => {
    if (!available) {
      throw new IntegrationError("revenuecat_preview", "Subscription management requires a development build.");
    }
    let customerInfo: CustomerInfo;
    try {
      customerInfo = await Purchases.getCustomerInfo();
    } catch (cause) {
      throw toIntegrationError(cause, "customer_info_failed", "Could not load subscription details.");
    }
    if (!customerInfo.managementURL) {
      throw new IntegrationError("management_unavailable", "No subscription management URL is available.");
    }
